 ...
 * [Gyroscope.OnFragmentInteractionListener] interface
 * to handle interaction events.
 * Use the [Gyroscope.newInstance] factory method to
 * create an instance of this fragment.
 */
class Gyroscope : Fragment() {
    ...
    override fun onCreateView(
        ...
        view.card.floatingActionButton.setOnClickListener {
            ...
            if(enableLog){
                ...
                intent.putExtra(Intent.EXTRA_TITLE, getString(R.string.sensorGyroscope) + ".txt")
                startActivityForResult(intent, 1)
            }
        }

        view.card.imageView.setImageDrawable(ContextCompat.getDrawable(view.context, R.drawable.ic_gyroscope_white))
        activity!!.toolbar.setBackgroundColor(ResourcesCompat.getColor(resources, R.color.colorDeepPurple, null))
        view.card.backView.setBackgroundColor(ResourcesCompat.getColor(resources, R.color.colorDeepPurple, null))
        view.card.card_title.visibility = TextView.GONE
        //view.card.card_title.text = getString(R.string.sensorGyroscope)
        ...
        view.detail.sensor_text_max_range.text = "${getString(R.string.labelMaxRange)} ${mSensor.maximumRange} ${getString(R.string.unitRadiantSecond)}"
        view.detail.sensor_text_information.text = getString(R.string.infoGyroscope)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        sEventListener = SensorSingleton.registerSelected(sensorManager, mSensor) {
            view.card_v1.text = String.format("%.2f", it.values[0]) + " ${getString(R.string.unitRadiantSecond)}"
            view.card_v2.text = String.format("%.2f", it.values[1]) + " ${getString(R.string.unitRadiantSecond)}"
            view.card_v3.text = String.format("%.2f", it.values[2]) + " ${getString(R.string.unitRadiantSecond)}"
    ...
    companion object {
         ...
         * @return A new instance of fragment Gyroscope.
         */
